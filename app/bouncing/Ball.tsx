...
const generateSpheres = (count: number) => {
    ...
    for (let i = 0; i < count; i++) {
        spheres.push([
            parseFloat(getRandomPosition(-5, 5)),  // x position
            parseFloat(getRandomPosition(5, 15)),  // y position
            parseFloat(getRandomPosition(-5, 5)),  // z position
        ]);
    }
    return spheres;
};
...
const Ball = ({ canvasRef }: BallProps) => {
    const spheres = generateSpheres(200); // Generate 10 spheres
